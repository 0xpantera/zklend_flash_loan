...
pub trait IMarket<TContractState> {
    ...
    fn get_user_debt_for_token(self: @TContractState, user: ContractAddress, token: ContractAddress) -> felt252;

    /// Returns a bitmap of user flags.
    fn get_user_flags(self: @TContractState, user: ContractAddress) -> felt252;

    fn is_user_undercollateralized(self: @TContractState, user: ContractAddress, apply_borrow_factor: bool) -> bool;
    ...
    fn repay_for(ref self: TContractState, token: ContractAddress, amount: felt252, beneficiary: ContractAddress);
    ...
    fn liquidate(
        ref self: TContractState, user: ContractAddress, debt_token: ContractAddress, amount: felt252, collateral_token: ContractAddress
    );

    fn flash_loan(ref self: TContractState, receiver: ContractAddress, token: ContractAddress, amount: felt252, calldata: Span::<felt252>);
    ...
    fn set_collateral_factor(ref self: TContractState, token: ContractAddress, collateral_factor: felt252);
...
trait IZToken<TContractState> {
    ...
    fn felt_allowance(self: @TContractState, owner: ContractAddress, spender: ContractAddress) -> felt252;
    ...
    fn felt_transfer_from(ref self: TContractState, sender: ContractAddress, recipient: ContractAddress, amount: felt252) -> bool;
    ...
    fn move(ref self: TContractState, from_account: ContractAddress, to_account: ContractAddress, amount: felt252);
...
#[starknet::interface]
pub trait IZklendFlashCallback<TContractState> {
    fn zklend_flash_callback(ref self: TContractState, caller: ContractAddress, calldata: Span::<felt252>);
}
...
trait IInterestRateModel<TContractState> {
    ...
    fn get_interest_rates(self: @TContractState, reserve_balance: felt252, total_debt: felt252) -> ModelRates;
}
